...
void check_negation(stack<char>& op_check, string& postfix){
    if(op_check.size() == 0)return;//iF NEGATION IS THE FIRST ELEMENT WE DON'T WANT TO PRINT IT
    if(op_check.top() == '~'){
...
void check_bracket(stack<char>& op_brack, string& postfix){
    if(op_brack.top() == ')'){//If i find right bracket 
        op_brack.pop();//first remove it
        while(op_brack.top() != '('){//till i dont find left bracket
